	...
	while (awake) {
		...
		if (token.action == back) {
			if (token.id == node_id) {
				...
				if (my_zmq::send_recieve_wait(token_down, reply_down, node_socket) and reply_down.action == success) {
					*reply = reply_down;
				}
			}
			...
			if (my_zmq::send_recieve_wait(token_ping, reply_ping, node_socket) and reply_ping.action == success) {
				reply->action = success;
			}
			...
			if (token.parent_id == node_id) {
				...
				if (fork_id == 0) {
					...
					ok = my_zmq::recieve_msg_wait(reply_info, node_socket);
					...
					if (has_child) {
						...
						ok = my_zmq::send_recieve_wait(token_bind, reply_bind, node_socket);
					...
					if (ok) {
						...
						ok = my_zmq::send_recieve_wait(token_ping, reply_ping, node_socket);
			...
			if (has_child) {
				if (token.id == child_id) {
					...
					ok = my_zmq::send_recieve_wait(token_down, reply_down, node_socket);
