	...
	int main() {
		...
		std::vector< std::pair<void*, void*> > childs; /*для хранения контекстов и сокетов для дочерних процессов*/
		...
		while (std::cin >> s >> id) {
			if (s == "create") {
				...
				if (parent_id == -1) {
					...
					/*привязка сокета к TCP-адресу с использованием порта, 
					который вычисляется как сумма PORT_BASE и id.*/
					assert(rc == 0);
					...
					if (fork_id == 0) { 
						...
						ok = my_zmq::recieve_msg_wait(reply_info, new_socket);

						node_token_t* token = new node_token_t({ping, id, id});
						node_token_t reply({fail, id, id});
						ok = my_zmq::send_recieve_wait(token, reply, new_socket);
					...
					if (control_node.find(id) != -1) {
						...
						node_token_t reply({fail, id, id});
						if (my_zmq::send_recieve_wait(token, reply, childs[ind].second) and reply.action == success) {
				...
				if (ind != -1) {
					...
					node_token_t reply({fail, id, id});
					bool ok = my_zmq::send_recieve_wait(token, reply, childs[ind].second);
					if (reply.action == destroy and reply.parent_id == id) {
						...
						rc = zmq_ctx_term(childs[ind].first);
						assert(rc == 0);
						my_zmq::init_pair_socket(childs[ind].first, childs[ind].second);
				...
				if (ind != -1) {
					...
					node_token_t reply({fail, id, id});
					if (my_zmq::send_recieve_wait(token, reply, childs[ind].second)) {
						if (reply.action == success) {
							...
							while (my_zmq::send_recieve_wait(token_back, reply_back, childs[ind].second) and reply_back.action == success) {
				...
				if (ind != -1) {
					...
					for (size_t i = 0; i < text_pattern.size(); ++i) {
						...
						node_token_t reply({fail, id, id});
						if (!my_zmq::send_recieve_wait(token, reply, childs[ind].second) or reply.action != success) {
							ok = false;
							break;
						}
					}
